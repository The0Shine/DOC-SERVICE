package com.horob1.doc_service.api.controller

import com.horob1.doc_service.api.dto.response.DocumentSummaryResponse
import com.horob1.doc_service.service.query.BookmarkQueryHandler
import com.horob1.doc_service.shared.dto.response.ApiResponse
import org.springframework.http.HttpStatus
import org.springframework.http.ResponseEntity
import org.springframework.security.core.context.SecurityContextHolder
import org.springframework.web.bind.annotation.*
import java.util.UUID

@RestController
@RequestMapping("/api/v1/bookmarks")
class BookmarkQueryController(
    private val bookmarkQueryHandler: BookmarkQueryHandler
) {
    @GetMapping
    fun getMyBookmarks(): ResponseEntity<ApiResponse<List<DocumentSummaryResponse>>> {
        val userId = SecurityContextHolder.getContext().authentication.principal as UUID

        val apiResponse = ApiResponse.success(
            message = "Get bookmarks successfully",
            status = HttpStatus.OK,
            data = bookmarkQueryHandler.getMyBookmarks(userId)
        )
        return ResponseEntity.status(apiResponse.status).body(apiResponse)
    }
}
